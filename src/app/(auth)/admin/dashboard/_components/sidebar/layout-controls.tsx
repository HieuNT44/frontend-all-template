...
import { setValueToCookie } from "@/services/server-actions";
import { usePreferencesStore } from "@/stores/preferences-provider";
...
import { updateContentLayout, updateNavbarStyle } from "@/utils/layout-utils";
...
export function LayoutControls(props: LayoutControlsProps) {
  ...
  const handleValueChange = async (key: string, value: string | null) => {
    if (!value) return;

    if (key === "theme_mode") {
      const themeModeValue = value as ThemeMode;
      updateThemeMode(themeModeValue);
      setThemeMode(themeModeValue);
    }

    if (key === "theme_preset") {
      const themePresetValue = value as ThemePreset;
      updateThemePreset(themePresetValue);
